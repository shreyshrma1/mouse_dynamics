...
def extract_session_features(filepath, user_id, window_size=None):
    """
    Extract features from a single session file.

    If window_size is set, actions are sliced into fixed-size windows of
    that many actions, and each window gets a unique session label
    (filename_w0, filename_w1, ...). This produces more pseudo-sessions
    for leave-one-out evaluation.

    If window_size is None, all actions share the same session label
    (the filename).
    """
    ...
    if window_size is None:
        # original behaviour — one session per file
        ...
        # slice actions into windows of window_size
...
def main():
    ...
    parser.add_argument('--window_size', type=int, default=None,
                        help='If set, slice each session into windows of this many actions '
                             '(e.g. 200). Produces more pseudo-sessions for LOO evaluation.')
