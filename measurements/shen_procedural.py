...
Optimizations:
  - numba @njit JIT-compiled DTW (near-C speed after first call)
  - All user windows precomputed once before the main evaluation loop

For each window the distance vector has two parts:
  1. Holistic (39-dim): Manhattan distance between the window's 39-dim
     holistic vector and the reference window's holistic vector.
  2. Procedural (2 * window_size scalars): DTW distance between each
     action's speed/acceleration curve and the corresponding curve in
...
def parse_args():
    ...
    parser.add_argument("--max_impostor_windows", type=int,   default=30,
                        help="Max windows to sample per impostor user (default: 30)")
...
def dtw_distance(a, b):
    """JIT-compiled DTW distance. Slow on first call (compilation), fast after."""
    n, m = len(a), len(b)
...
def compute_distance_vector(window, ref_window):
    ...
    new_curves = window["curves"]
    ref_curves = ref_window["curves"]
    n_curves   = min(len(new_curves), len(ref_curves))

    procedural_dists = []
    for i in range(n_curves):
        procedural_dists.append(dtw_distance(
            new_curves[i][0].astype(np.float64),
            ref_curves[i][0].astype(np.float64)))
        procedural_dists.append(dtw_distance(
            new_curves[i][1].astype(np.float64),
            ref_curves[i][1].astype(np.float64)))

    expected = 2 * len(ref_curves)
    while len(procedural_dists) < expected:
        procedural_dists.append(0.0)

    return np.concatenate([holistic_dist, np.array(procedural_dists)])
...
def main():
    ...
    # Warm up numba JIT compilation before timing-sensitive work
    ...
    for user in BALABIT_USERS:
        ...
        print(f"  Reference: window {ref_idx}, "
              f"{len(ref_window['curves'])} actions, "
              f"distance vector dim: {39 + 2 * len(ref_window['curves'])}")

        print(f"  Computing training distance vectors...")
        ...
        print(f"  Computing test distance vectors...")
