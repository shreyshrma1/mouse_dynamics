...
Evaluates UMAP + LOF on the Balabit dataset.

Pipeline per user:
  1. Extract holistic + scroll feature windows (same as shen_replication_scroll)
  2. Standardize (fit on training windows only)
  3. Fit UMAP on training windows → reduce to n_components dimensions
  4. Fit LOF (novelty=True) on UMAP-reduced training windows
  5. Evaluate on held-out legitimate + all impostor windows

No distance-vector preprocessing — features go straight into UMAP then LOF.
LOF scores: negative = outlier (rejected), positive = inlier (accepted).

Usage:
    python measurements/umap_lof.py
    python measurements/umap_lof.py --n_components 10 --n_neighbors 15
"""
...
def parse_args():
    ...
    parser.add_argument("--n_components",  type=int,   default=10,
                        help="UMAP output dimensions (default: 10)")
    parser.add_argument("--n_neighbors",   type=int,   default=15,
                        help="UMAP n_neighbors (default: 15)")
    parser.add_argument("--min_dist",      type=float, default=0.1,
                        help="UMAP min_dist (default: 0.1)")
    parser.add_argument("--lof_neighbors", type=int,   default=20,
                        help="LOF n_neighbors (default: 20)")
    parser.add_argument("--contamination", type=float, default=0.1,
                        help="LOF contamination (default: 0.1)")
...
def score_windows(vecs, scaler, reducer, model):
    if len(vecs) == 0:
        return np.array([])
...
def main():
    ...
    print(f"LOF:  n_neighbors={args.lof_neighbors}, contamination={args.contamination}")
    print(f"window_size={args.window_size}, held_out_frac={args.held_out_frac}\n")
    ...
    for user in BALABIT_USERS:
        ...
        # ── UMAP ──────────────────────────────────────────────────────────
        # n_neighbors must be < n_train
        n_neighbors = min(args.n_neighbors, n_train - 1)
        n_components = min(args.n_components, n_train - 1)
        reducer = umap.UMAP(
        ...
        # ── LOF ───────────────────────────────────────────────────────────
        lof_neighbors = min(args.lof_neighbors, n_train - 1)
        model = LocalOutlierFactor(
            n_neighbors=lof_neighbors,
            novelty=True,
            contamination=args.contamination,
        )
        model.fit(train_umap)

        train_scores = model.decision_function(train_umap)
        print(f"  Train scores: min={train_scores.min():.4f}, "
              f"mean={train_scores.mean():.4f}, max={train_scores.max():.4f}")

        # Accepted when score >= 0
        legit_scores   = score_windows(test_samples, scaler, reducer, model)
        ...
        for other_user in BALABIT_USERS:
            ...
            imp_scores  = score_windows(other_vecs, scaler, reducer, model)
            if len(imp_scores) == 0:
                continue
