...
Scroll features (7 + 3 burst = 10 additional columns):
    ...
    scroll_burst_count    – number of bursts (gap > BURST_THRESHOLD_MS apart)
...
MORE_SCROLL_COLS = [
    'scroll_dy_abs_mean',       # mean absolute deltaY per event
    'scroll_dy_abs_std',        # std  absolute deltaY per event
    'scroll_dy_abs_max',        # max  absolute deltaY in window
    'scroll_dy_total',          # total scroll distance (sum of abs deltaY)
    'scroll_dy_up_ratio',       # weighted up/down ratio by deltaY magnitude
    'scroll_speed_mean',        # mean deltaY / inter-event-time
    'scroll_speed_std',         # std  of scroll speed
    'scroll_accel_mean',        # mean change in scroll speed between events
    'scroll_accel_std',         # std  of scroll acceleration
    'scroll_burst_dy_mean',     # mean total deltaY per burst
    'scroll_burst_dy_per_tick', # mean deltaY per tick within a burst
    'scroll_speed_intensity',   # mean abs(deltaY) / inter-scroll-interval
]
...
def extract_scroll_features(raw_df, window_start_idx, window_end_idx,
    ...
    more_scroll          – if True, also compute deltaY-based features
                           (requires y column to contain deltaY, not 0)

    Returns a dict with keys matching SCROLL_COLS (+ MORE_SCROLL_COLS if more_scroll).
    """
    ...
    # Scroll events: button=='Scroll', state=='Down'|'Up'
    ...
    dy = scrolls['y'].values.astype(float)  # raw deltaY per scroll event
    dy_abs = np.abs(dy)
    ...
    # Weighted up/down ratio by magnitude
    ...
    if scroll_count >= 2:
        ...
        # Speed-intensity: mean(abs(deltaY) / gap) — same as speed_mean but
        # kept separate to match the described feature
    ...
    # Burst deltaY features
    burst_dy_totals    = [float(np.sum(np.abs(dy[s:e]))) for s, e in bursts]
    burst_dy_per_tick  = [
        float(np.mean(np.abs(dy[s:e]))) if e > s else 0.0
        for s, e in bursts
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description='Extract 39 mouse dynamics + 9 scroll features from Balabit-format data.')
    ...
    if args.mode == 'dataset':
        ...
        for user in users:
            ...
            df = extract_user_features(user_dir, user, window_size=args.window_size)
