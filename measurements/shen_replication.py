...
Replicates the Shen et al. (2013) pipeline as closely as possible:

  For each user:
    1. Extract all windows across all sessions (one feature vector per window)
    2. Split: first 75% of windows = train, last 25% = test
    3. Find the reference vector (training sample with min mean distance to others)
    4. Compute distance vectors to reference, normalize with training stats
...
Usage:
    ...
    python measurements/shen_replication.py --nu 0.06 --gamma scale
"""
