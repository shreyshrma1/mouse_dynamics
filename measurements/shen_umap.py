...
Same as shen_replication_scroll.py but adds UMAP dimensionality reduction
between standardization and the Shen distance preprocessing.

Pipeline per user:
  raw features → StandardScaler → UMAP → find_reference →
  distance vectors → normalize → OCSVM

Usage:
    python measurements/shen_umap.py
    python measurements/shen_umap.py --n_components 10 --n_neighbors 15
"""
...
def score_windows(vecs, scaler, reducer, reference, dist_mean, dist_std, model):
    if len(vecs) == 0:
        return np.array([])
...
def main():
    ...
    print(f"Pipeline: StandardScaler → UMAP → find_reference → distance vectors → OCSVM\n")
    ...
    for user in BALABIT_USERS:
        ...
        reference   = find_reference(train_umap)
        train_dists = distance_vectors(train_umap, reference)
        ...
        legit_scores   = score_windows(test_samples, scaler, reducer,
                                       reference, dist_mean, dist_std, model)
        ...
        for other_user in BALABIT_USERS:
            ...
            imp_scores  = score_windows(other_vecs, scaler, reducer,
                                        reference, dist_mean, dist_std, model)
            if len(imp_scores) == 0:
                continue
