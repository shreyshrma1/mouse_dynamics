"""
eval_shen_scroll_user.py

Identical to eval_nn_manhat_user.py but uses the OCSVM model trained by
train_from_existing_shen_scroll.py instead of the NN Manhattan detector.

Evaluates against a single specified impostor user. Uses either the EER
threshold saved by evaluate_shen_scroll.py (--threshold flag) or computes
the optimal threshold fresh from the chosen impostor user.

Usage:
    python measurements/eval_shen_scroll_user.py
    python measurements/eval_shen_scroll_user.py --threshold
    python measurements/eval_shen_scroll_user.py
        --impostor_dir collected_data --threshold
"""
...
def parse_args():
    ...
    parser.add_argument("--threshold", action="store_true",
                        help="Use the EER threshold saved in state.pkl (computed by "
                             "evaluate_shen_scroll.py against Balabit). If omitted, the "
...
def main():
    ...
    if args.threshold:
        if saved_threshold is None:
            ...
            threshold_source = "saved (from evaluate_shen_scroll.py vs Balabit)"
    ...
    print(f"""
{'=' * 45}
Results for '{user}' vs '{imp_user}'  (features: {feature_desc})
{'=' * 45}
