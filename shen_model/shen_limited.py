...
Identical to shen_replication_scroll.py, but restricted to the top 15
features identified by permutation importance (feat_importance.py).

Top 15 features (ranked by mean AUC drop):
...
Usage:
    python measurements/shen_limited.py
    python measurements/shen_limited.py --nu 0.06 --gamma scale
"""
...
# All 48 features — needed for extraction; we subset after
HOLISTIC_COLS = [
...
# Top 15 from permutation importance — model uses only these
TOP_FEATURES = [
    "num_critical_points",
    "num_points",
    "sum_of_angles",
...
# Indices into ALL_FEATURE_COLS — sliced to --top_n at runtime (see main)
...
def extract_all_windows(session_files, user_id, window_size):
    """Extract windows and immediately subset to TOP_FEATURES."""
    all_vecs = []
...
def main():
    ...
    for user in BALABIT_USERS:
        ...
        model = OneClassSVM(kernel="linear", nu=args.nu, gamma=gamma)
    ...
    print(f"\n{'=' * 62}")
    print(f"  Aggregate Results ({len(all_results)} users) — top-{top_n} features")
    print(f"{'=' * 62}")
