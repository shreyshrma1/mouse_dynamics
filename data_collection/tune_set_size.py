...
Determines the minimum number of windows per set needed to reliably
distinguish legitimate users from impostors in the consensus authentication
system, without requiring any impostor data for threshold calibration.

For each set size from MIN_SET_SIZE to MAX_SET_SIZE:
  1. Calibrate the consensus threshold from the first half of the legitimate
     user's held-out windows (same approach as bank_to_bank_1d_tune.py).
     Threshold = one discrete step below the minimum observed acceptance rate.
  2. Evaluate over N_RUNS runs using the second half of held-out windows
     as legitimate sets and contiguous blocks from a single bank impostor user.
  3. Record FAR, FRR, and raw accepted/rejected counts at the calibrated threshold.

Output is a table showing how performance improves as set size increases,
making it easy to identify the minimum viable set size for deployment.

Usage:
    python data_collection/tune_set_size.py
    python data_collection/tune_set_size.py --runs 50
    python data_collection/tune_set_size.py --runs 50 --fixed
    python data_collection/tune_set_size.py --runs 50 --min_size 3 --max_size 15
"""
...
def calibrate_threshold(m, set_size, no_step=False):
    """
    Derive the consensus threshold from the calibration windows without
    requiring any impostor data.

    Slides a window of set_size across the calibration windows, records
    the acceptance rate of each block, then sets the threshold one discrete
    step below the minimum observed rate. This guarantees the legitimate user
    clears the bar while giving impostors a meaningful hurdle.
    """
    ...
    threshold = max(0.0, min(1.0, min_rate if no_step else min_rate - step))
...
def generate_sets(legit_windows, impostor_windows, set_size, rng):
    sets = []
    ...
    for i in range(N_IMPOSTOR):
        block, _ = sample_contiguous_block(impostor_windows, set_size, rng)
...
def evaluate_set_size(m, impostor_windows, set_size, threshold, runs, base_seed, fixed):
    legit_windows = m["eval_samples"]

    if len(legit_windows) < set_size or len(impostor_windows) < set_size:
        return None
    ...
    for run_idx in range(runs):
        ...
        sets = generate_sets(legit_windows, impostor_windows, set_size, rng)
...
def main():
    parser = argparse.ArgumentParser(
        description="Find minimum set size for reliable consensus authentication")
    parser.add_argument("--runs",     type=int, default=DEFAULT_RUNS,
                        help=f"Number of evaluation runs per set size (default: {DEFAULT_RUNS})")
    ...
    parser.add_argument("--no_step",  action="store_true",
                        help="Set threshold equal to the minimum observed acceptance rate "
                             "rather than one step below it. Stricter — prefers low FAR "
                             "over low FRR.")
    parser.add_argument("--min_size", type=int, default=MIN_SET_SIZE,
                        help=f"Minimum set size to test (default: {MIN_SET_SIZE})")
    parser.add_argument("--max_size", type=int, default=MAX_SET_SIZE,
                        help=f"Maximum set size to test (default: {MAX_SET_SIZE})")
    ...
    print(f"\n[Config] Model={user_a}  Impostor={user_b}  Runs={args.runs}  "
          ...
          f"Step={'off' if args.no_step else 'on'}  "
    ...
    for set_size in range(args.min_size, args.max_size + 1):
        threshold = calibrate_threshold(model_a, set_size, args.no_step)

        result = evaluate_set_size(
            model_a, impostor_windows, set_size, threshold,
